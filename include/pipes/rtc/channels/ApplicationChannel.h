...
namespace rtc {
	...
	class DataChannel {
			friend class ApplicationChannel;
		public:
			...
			void close();
		private:
			DataChannel(ApplicationChannel*, uint16_t id, std::string lable, std::string protocol);

			bool read = true, write = true;
			ApplicationChannel* owner;
			uint16_t _id;
			std::string _lable;
			std::string _protocol;
	};

	class ApplicationChannel : public Channel {
		...
		public:
			...
			ApplicationChannel(PeerConnection* /* owner */, NiceStreamId /* channel id */, const std::shared_ptr<Configuration>& /* configuration */);
			virtual ~ApplicationChannel();
		...
		private:
		protected:
			void on_dtls_initialized(const std::shared_ptr<DTLSHandler>&certificate) override;
